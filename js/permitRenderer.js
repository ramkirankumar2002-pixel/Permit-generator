...
function qrDateTimeParts(dt){
  const s = String(dt || '').trim();
  let datePart = '';
  const iso = s.match(/(\d{4})-(\d{2})-(\d{2})/);
  const dmy = s.match(/(\d{1,2})[\/\-.](\d{1,2})[\/\-.](\d{4})/);
  if (iso) datePart = `${iso[3]}-${iso[2]}-${iso[1]}`;
  else if (dmy) datePart = `${dmy[1].padStart(2,'0')}-${dmy[2].padStart(2,'0')}-${dmy[3]}`;
  const tm = s.match(/(\d{1,2}):(\d{2})/);
  const timePart = tm ? `${String(tm[1]).padStart(2,'0')}:${tm[2]}` : '';
  return { datePart, timePart };
}

function buildQRData(p) {
  const { datePart, timePart } = qrDateTimeParts(p.dispatchDateTime);
  const distNum = String(p.totalDistance || '').replace(/[^\d.]/g, '');
  const qtyNum = String(p.quantity || '').replace(/[^\d.]/g, '');
  ...
  return [
    ...
    distNum ? `${distNum}kms` : '',
    hrs,
    mineral && qtyNum ? `${mineral}(${qtyNum}MT)` : mineral,
    compactQRToken(p.vehicleNo).toUpperCase(),
